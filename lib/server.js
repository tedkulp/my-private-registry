...
module.exports = (() => {
  ...
  var run = args => {
    ...
    app.delete('/v2/:repository/manifests/:reference', (req, res) => {
      ...
      return registry.getManifestDetails(req.params['repository'], req.params['reference'])
        .then(details => registry.deleteManifest(req.params['repository'], req.params['reference']))
    ...
    //       real API spec
    app.post('/v2/:repository/manifests/copy', (req, res) => {
      console.log(req.url, req.method, req.headers, req.body);

      return registry.getManifestDetails(req.params['repository'], req.body.reference)
        .then(existingDetails => {
          return registry.copyManifest(req.params['repository'], req.body.reference, req.body.newReference)
            .then(() => {
              return registry.getManifestDetails(req.params['repository'], req.body.newReference)
                .then(newDetails => {
                  return res
                    .append('Location', '/v2/' + req.params['repository'] + '/manifests/' + req.body.newReference)
                    .append('Content-Length', 0)
                    .append('Docker-Content-Digest', newDetails.digest)
                    .status(201).end();
                })
            })
        })
